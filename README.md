...
根據您提供的專案原始碼與 `README.md` 文件，`main.py` 與 `pack_apk.py` 的功能分析如下：

1. main.py

`main.py` 是基於 **FastAPI** 框架開發的後端服務，負責處理來自前端（Cloudflare Pages）的網路請求，並管理 APK 處理的生命週期。

**API 介面提供**：
* `POST /api/upload`：接收使用者上傳的 APK 檔案，為其生成唯一的 `task_id`（UUID），並啟動背景處理任務。
* `GET /api/status/{task_id}`：讓前端輪詢目前的處理進度（如：`protecting`、`aligning`、`signing` 等）。
* `GET /api/download/{task_id}`：當處理完成後，提供保護後的 APK 檔案下載。


**背景任務管理**：
* 使用 `BackgroundTasks` 在不阻塞 API 回應的情況下執行繁重的 APK 處理邏輯。
* 負責建立並清理臨時目錄（`uploads` 和 `protected`）。


**腳本調用**：
* 定義 `run_apk_hardening_script` 函數，負責呼叫外部的 Python 環境來執行 `pack_apk.py`，並傳遞輸入路徑、輸出路徑及任務 ID 作為參數。

2. pack_apk.py

`pack_apk.py` 是專案的核心，負責執行實際的 Android APK 逆向工程、加密與重新打包流程。

**自動化加殼流程**：
1. **解包與分析**：利用 `apktool` 將原始 APK 解開，並讀取其中的 `classes.dex`。
2. **DEX 加密**：使用 AES-GCM 演算法加密原始的 `classes.dex` 檔案，並將加密後的數據存放在 `assets` 目錄中。
3. **清單修補（Manifest Patching）**：自動修改 `AndroidManifest.xml`，將應用程式的進入點（`android:name`）替換為預先準備好的 `LOADER_APPLICATION_NAME`（加殼用的 Stub）。
4. **注入 Stub**：將自定義的 Loader DEX 放入 APK 中，取代原本的程式碼邏輯，負責在執行時動態解密並載入原始程式碼。
5. **對齊與簽章**：執行 `zipalign` 優化檔案結構，並使用 `apksigner` 進行數位簽章，確保 APK 可以在 Android 裝置上正常安裝與運行。

**進度回報**：
* 定義 `update_status` 函數，將目前的處理階段（如 `aligning` 或 `signing`）寫入到以 `task_id` 命名的狀態檔案中，供 `main.py` 讀取並回傳給前端。
